...
async function run() {
    ...
    const staleRows = stale.rows;
    console.log(`  total rows in starter_settings: ${total.rows[0].count}`);
    ...
    for (let i = 0; i < staleRows.length; i += BATCH_SIZE) {
        ...
        const result = await db.execute(sql `DELETE FROM starter_settings WHERE id IN (${sql.join(ids.map((id) => sql `${id}`), sql `, `)})`);
