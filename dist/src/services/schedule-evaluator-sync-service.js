...
export async function runScheduleEvaluator(schedules) {
    if (!schedules.length)
        return [];
    const now = new Date();
    const transitions = [];
    for (const s of schedules) {
        ...
        if (!res || res.newStatus === s.schedule_status)
            continue;
    ...
    await Promise.all(transitions.map(async (t) => {
        const s = t.schedule;
    ...
    if (appliedTransitions.length > 0) {
        await db.insert(motorScheduleLogs).values(appliedTransitions.map(t => ({
