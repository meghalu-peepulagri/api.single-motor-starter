...
export async function getCompletePayloadAnalytics(filters) {
    ...
    const overallStats = await db.select({
        total_records: sql `count(*)::int`,
        valid_records: sql `count(*) filter (where ${starterBoxParameters.payload_valid} = true)::int`,
        invalid_records: sql `count(*) filter (where ${starterBoxParameters.payload_valid} = false)::int`,
