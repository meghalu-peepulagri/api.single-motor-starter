 ...
 */
function buildConvertedConfig(flatSettings, defaultSettings, existingMotor, newMotorId) {
    ...
    const defaultsSource = (defaultSettings?.multi_motor_defaults?.motor ?? defaultSettings ?? {});
    const m1 = {
    ...
    return {
        ...
        v_flt_en: defaultSettings?.multi_motor_defaults?.v_flt_en ?? 0,
 ...
 */
export async function convertStarterToDualMotor(starter, existingMotor, payload, performerId) {
    ...
    const ackedSettings = await db.query.starterSettings.findFirst({
        where: and(eq(starterSettings.starter_id, starter.id), eq(starterSettings.acknowledgement, "TRUE"), eq(starterSettings.is_new_configuration_saved, 1)),
        orderBy: desc(starterSettings.created_at),
    });
    return await db.transaction(async (trx) => {
        const newMotorPayload = {
            name: payload.name,
        ...
        const motor = await saveSingleRecord(motors, newMotorPayload, trx);
        ...
        if (ackedSettings) {
            const config = buildConvertedConfig(ackedSettings, defaultSettings, existingMotor, motor.id);
...
export async function findSingleLiveMotor(starterId) {
...
export function armSettingsResyncAfterConversion(starterId) {
