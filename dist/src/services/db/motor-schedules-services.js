...
import { buildScheduleData, dateToYYMMDD, expandDateRangeByDays, nextDayYYMMDD, normalizeMotorSchedulePayload, todayAsYYMMDD } from "../../helpers/motor-schedule-payload-helper.js";
import { checkIntraArrayConflicts, checkMotorScheduleConflict, timeToMinutes, validateScheduleTypeRules, wallClockMinutes } from "../../helpers/motor-helper.js";
...
export const SLOT_FREE_STATUSES = ["DELETED", "FAILED", "COMPLETED", "PARTIAL", "MISSED"];
...
export async function findSchedulesByFilters(filters, page = 1, limit = 10) {
    ...
    const rawRecords = await db
        ...
        .leftJoin(motors, eq(motorSchedules.motor_id, motors.id))
        .where(whereClause)
...
export async function findMaxAckedEndDatePerStarter(todayNum) {
    const rows = await db
        ...
        .from(motorSchedules)
        .where(and(eq(motorSchedules.acknowledgement, 1), gte(motorSchedules.schedule_end_date, todayNum), ne(motorSchedules.status, "ARCHIVED"), notInArray(motorSchedules.schedule_status, [...SLOT_FREE_STATUSES])))
...
export async function findPendingSchedulesForStarter(starterId, motorId) {
    ...
    const baseConditions = [
        ...
        or(gte(motorSchedules.schedule_start_date, todayNum), and(eq(motorSchedules.schedule_start_date, yesterdayNum), gte(motorSchedules.schedule_end_date, todayNum))),
    ];
    if (motorId != null) {
 ...
 */
export async function findAndDeleteExpiredSchedules(starterId, motorId) {
    const todayNum = dateToYYMMDD(new Date());
    const conditions = [
        eq(motorSchedules.starter_id, starterId),
    ...
    for (const record of expired) {
        ...
        const resolved = resolveTerminalStatus(record, startMinutes, endMinutes, now);
        resolvedById.set(record.id, resolved.newStatus);
    }
    ...
    await batchUpdateScheduleStatuses([...groups.entries()].map(([status, ids]) => ({ status: status, ids, last_stopped_at: now })));
    await db.insert(motorScheduleLogs).values(expired.map(record => ({
...
export async function findEvaluatableSchedules() {
    ...
    return await db.query.motorSchedules.findMany({
        where: and(eq(motorSchedules.enabled, true), ne(motorSchedules.status, "ARCHIVED"), inArray(motorSchedules.schedule_status, ["PENDING", "SCHEDULED", "RUNNING", "WAITING_NEXT_CYCLE", "PARTIAL"]), or(
...
export async function bulkCreateMotorSchedules(rawPayload, userId) {
    ...
    for (const item of preparedList) {
        ...
        const isOvernightSingleNight = timeToMinutes(item.end_time) <= timeToMinutes(item.start_time)
    ...
    const takenSlots = await db
        ...
        .from(motorSchedules)
        .where(and(eq(motorSchedules.motor_id, motorId), inArray(motorSchedules.schedule_id, incomingSlots), ne(motorSchedules.status, "ARCHIVED"), notInArray(motorSchedules.schedule_status, [...SLOT_FREE_STATUSES])));
    ...
    const finalPayload = expandedList.map((item) => ({
        ...
        device_schedule_id: item.device_schedule_id ?? null,
        created_by: userId,
        enabled: item.enabled ?? true,
        schedule_status: item.schedule_status ?? "PENDING",
    }));
    const result = await saveRecords(motorSchedules, finalPayload);
    ...
    const maxDeviceId = Math.max(0, ...finalPayload.map((p) => p.device_schedule_id ?? 0).filter(Boolean));
    if (maxDeviceId > 0) {
...
export async function assignDeviceScheduleIds(motorId, records) {
    ...
    await db.transaction(async (trx) => {
        ...
        const nullRows = await trx
            ...
            .from(motorSchedules)
            .where(and(inArray(motorSchedules.id, sorted.map(r => r.id)), isNull(motorSchedules.device_schedule_id)));
        ...
        const occupiedRows = await trx
            ...
            .from(motorSchedules)
            .where(and(eq(motorSchedules.motor_id, motorId), ne(motorSchedules.status, "ARCHIVED"), notInArray(motorSchedules.schedule_status, [...SLOT_FREE_STATUSES]), isNotNull(motorSchedules.device_schedule_id)));
...
export async function syncLastDeviceScheduleId(motorId) {
    const maxRow = await db
        .select({ maxId: sql `COALESCE(MAX(${motorSchedules.device_schedule_id}), 0)` })
        .from(motorSchedules)
        .where(and(eq(motorSchedules.motor_id, motorId), ne(motorSchedules.status, "ARCHIVED"), notInArray(motorSchedules.schedule_status, [...SLOT_FREE_STATUSES])));
