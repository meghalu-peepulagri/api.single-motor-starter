    ...
    try {
        ...
        if (typeof message.D === "object" && message.D !== null) {
            ...
            for (const ref of motorRefs) {
                ...
                if (notificationData)
                    notifications.push({ data: notificationData, mode });
            }
...
export async function heartbeatHandler(message, topic) {
    try {
        ...
        await db.transaction(async (trx) => {
            await updateRecordByIdWithTrx(starterBoxes, device.id, starterBoxUpdates, trx);
        ...
        if (isNowOnline) {
            const motorList = Array.isArray(device.motors) ? device.motors : [];
            setImmediate(() => {
                if (motorList.length === 0) {
                    pushPendingSchedulesForStarter(device).catch((err) => logger.error(`heartbeat schedule push failed for starter ${device.id}: ${err?.message}`));
                    return;
                }
                for (const motor of motorList) {
                    pushPendingSchedulesForStarter(device, motor.id).catch((err) => logger.error(`heartbeat schedule push failed for starter ${device.id} motor ${motor.id}: ${err?.message}`));
                }
            });
        }
    }
...
export async function publishData(preparedData, starterData) {
    if (!starterData)
        return null;
    ...
    let topic;
    if (starterData.role === "CHILD" && starterData.parent_starter_id) {
        const masterKey = await getMasterIdentifierById(starterData.parent_starter_id);
        if (!masterKey) {
 ...
 */
export async function insertParametersForUnmatchedMotor(device, motorRef, validated) {
    const starterId = device.id;
    ...
    const lastRecord = await db
        ...
        .where(and(eq(starterBoxParameters.starter_id, starterId), eq(starterBoxParameters.motor_reference, motorRef)))
        .orderBy(desc(starterBoxParameters.time_stamp))
        .limit(1);
    let motorId = lastRecord[0]?.motor_id ?? null;
    ...
    if (!motorId) {
        ...
        const motorRecord = await db
            .select({ id: motors.id })
            .from(motors)
            .where(and(eq(motors.starter_id, starterId), eq(motors.motor_index, motorIndex), ne(motors.status, "ARCHIVED")))
            .limit(1);
