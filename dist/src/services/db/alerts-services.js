...
export async function getConsecutiveGroupsPaginated(starterId, motorId, offset, limit, type) {
...
export async function getConsecutiveAlertsPaginated(starterId, motorId, offset, limit) {
    return getConsecutiveGroupsPaginated(starterId, motorId, offset, limit, 'alert');
}
export async function getConsecutiveFaultsPaginated(starterId, motorId, offset, limit) {
    return getConsecutiveGroupsPaginated(starterId, motorId, offset, limit, 'fault');
}
export async function getConsecutiveGroupsCount(starterId, motorId, type) {
    const codeColumn = type === "alert" ? "alert_code" : "fault_code";
    const descColumn = type === "alert" ? "alert_description" : "fault_description";
    const excludeValues = type === "alert"
        ? ["Unknown Alert", "No Alert"]
