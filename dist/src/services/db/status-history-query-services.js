...
export async function getMotorOnRuntime(filters) {
    ...
    const hasRealMotorActivity = dedupedMotorEvents.some((e, i) => e.status === "ON" && dedupedMotorEvents[i + 1]?.status === "OFF") || motorCarryOver;
    const deviceStaleInactive = preDeviceRecord?.status === "INACTIVE" && hasRealMotorActivity;
