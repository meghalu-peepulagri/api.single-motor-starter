...
import { REPLACE_DEVICE_ASSIGNED, REPLACE_PCB_DUPLICATE, REPLACE_PCB_MOTOR_TYPE_MISMATCH, REPLACE_PCB_SAME_AS_CURRENT, REPLACE_SPARE_DEVICE_ASSIGNED, REPLACE_STARTER_NUMBER_DUPLICATE, STARTER_BOX_NOT_FOUND } from "../../constants/app-constants.js";
...
const isAdminOnlyOnReadyOrTest = (performer, starter) => performer.user_type === "ADMIN" && starter.device_status !== "READY" && starter.device_status !== "TEST";
/**
 ...
 */
async function lockReplaceableStarter(trx, starterId, performer) {
...
async function ensurePcbNumberAvailable(trx, starter, newPcbNumber) {
 ...
 */
async function ensureNewPcbMatchesMotorType(trx, old, newPcbNumber) {
    ...
    if (previousArity !== oldArity) {
        throw new ReplacementAttemptError("FAILED", `PCB Number ${newPcbNumber} was previously used on a ${motorTypeLabel(previousArity)} device and cannot replace a ${motorTypeLabel(oldArity)} device`, new ConflictException(REPLACE_PCB_MOTOR_TYPE_MISMATCH, { field: "pcb_number" }), old);
    }
}
async function ensureStarterNumberAvailable(trx, starter, newStarterNumber) {
    const [used] = await trx.select({ id: starterBoxes.id }).from(starterBoxes)
...
async function deviceMotorArity(trx, device) {
    ...
    const [row] = await trx.select({ count: sql `count(*)` }).from(motors).where(motorFilter);
 ...
 */
async function findExactSpareDeviceMatch(trx, oldId, newStarterNumber, newPcbNumber) {
    const [match] = await trx.select().from(starterBoxes)
        .where(and(ne(starterBoxes.status, "ARCHIVED"), ne(starterBoxes.id, oldId), sql `lower(${starterBoxes.starter_number}) = ${newStarterNumber.toLowerCase()}`, sql `lower(${starterBoxes.pcb_number}) = ${newPcbNumber.toLowerCase()}`))
        .limit(1);
    return match;
}
/** Shared by both box-replacement paths: retires the old device's motors, then the device itself. */
async function retireOldDeviceAndMotors(trx, old) {
    ...
    await updateRecordById(starterBoxes, old.id, { status: "ARCHIVED", device_status: "REPLACED" }, trx);
}
/**
...
function carryForwardBoxLevelSettings(oldSettings) {
    if (!oldSettings)
        return {};
    const out = {};
    for (const field of CARRY_FORWARD_ON_BOX_REPLACEMENT_FIELDS) {
        const value = oldSettings[field];
 ...
 */
function carryForwardMultiMotorConfig(oldSettings, defaultMultiMotorDefaults, newMotorsByIndex) {
    const oldBlocksByIndex = new Map((oldSettings?.multi_motor_config?.motors ?? []).map((block) => [block.motor_index, block]));
    ...
    for (const [motorIndex, newMotor] of newMotorsByIndex) {
        ...
        const source = oldBlock ? perMotorFields(oldBlock) : perMotorFields(defaultMotorTemplate);
        motorBlocks.push({
            ...source,
    ...
    return {
        ...
        v_flt_en: oldSettings?.multi_motor_config?.v_flt_en ?? defaultMultiMotorDefaults?.v_flt_en ?? 0,
        sd_time: 0,
        motors: motorBlocks,
    };
}
...
export async function replacePcbWithTransaction(input) {
    return await runLoggedReplacement("PCB", input, async (trx, remember) => {
        ...
        const updated = await updateRecordById(starterBoxes, starter.id, { pcb_number: input.newPcbNumber }, trx);
...
export async function replaceBoxWithTransaction(input) {
    return await runLoggedReplacement("BOX", input, async (trx, remember) => {
        ...
        if (!defaultSettings[0] || !defaultSettingsLimits[0])
            throw new Error("Default starter settings are not configured");
        ...
        const oldAckedSettings = await trx.query.starterSettings.findFirst({
            where: and(eq(starterSettings.starter_id, old.id), eq(starterSettings.acknowledgement, "TRUE"), eq(starterSettings.is_new_configuration_saved, 1)),
        ...
        await retireOldDeviceAndMotors(trx, old);
        const device = await saveSingleRecord(starterBoxes, {
            name: old.name,
        ...
        const newMotorsByIndex = new Map();
        ...
        const multiMotorConfig = oldArity === "MULTIPLE_MOTORS"
            ? carryForwardMultiMotorConfig(oldAckedSettings, defaultSettingsData?.multi_motor_defaults, newMotorsByIndex)
            : undefined;
        ...
        await saveSingleRecord(starterSettingsLimits, { ...defaultSettingsLimitsData, starter_id: device.id }, trx);
