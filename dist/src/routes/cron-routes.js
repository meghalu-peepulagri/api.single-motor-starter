...
cronRoutes.post("/schedule-sync-evening", async (c) => {
    runScheduleSync("evening-cron").catch(err => logger.error(`[cron:evening] ${err?.message}`));
...
cronRoutes.post("/schedule-sync-midnight", async (c) => {
    runScheduleSync("midnight-cron").catch(err => logger.error(`[cron:midnight] ${err?.message}`));
