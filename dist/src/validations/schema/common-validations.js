...
const phoneValidator = v.pipe(v.string(PHONE_NUMBER_REQUIRED), v.trim(), v.nonEmpty(PHONE_NUMBER_REQUIRED), v.regex(/^\d+$/, INVALID_PHONE_NUMBER), v.regex(/^[6-9]\d{9}$/, INVALID_PHONE_NUMBER_VALID_LENGTH), v.regex(/^(?!([6-9])\1{9}$).+$/, INVALID_PHONE));
const emailValidator = v.optional(v.pipe(v.string(EMAIL_REQUIRED), v.transform(value => value.trim()), v.nonEmpty(EMAIL_REQUIRED), v.email(VALID_MAIL)));
...
const starterBoxTitleValidator = v.optional(v.pipe(v.string(DEVICE_NAME_REQUIRED), v.transform(value => value.trim()), v.nonEmpty(DEVICE_NAME_REQUIRED), v.regex(/^[A-Z].*$/i, DEVICE_NAME_STARTS_LETTER), v.minLength(3, DEVICE_MIN_LEN)));
const macAddressValidator = v.optional(v.pipe(v.string(MAC_REQUIRED), v.transform(value => value.trim()), v.nonEmpty(MAC_REQUIRED), v.regex(/^([0-9A-Z:]+)$/, "Small letters or spaces are not allowed"), v.minLength(3, MAC_MIN_LEN)));
...
const pcbNumberValidator = v.optional(v.pipe(v.string(PCB_NUMBER_REQUIRED), v.transform(value => value.trim()), v.nonEmpty(PCB_NUMBER_REQUIRED), v.regex(/^[A-Z0-9]+$/, SMALL_LETTERS_NOT_ALLOWED), v.minLength(3, PCB_MIN_LEN)));
