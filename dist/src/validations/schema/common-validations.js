...
import { DEVICE_MIN_LEN, DEVICE_NAME_REQUIRED, DEVICE_NAME_STARTS_LETTER, EMAIL_REQUIRED, FIELD_NAME_INVALID, FIELD_NAME_MIN_LEN, FIELD_NAME_MUST_STRING, FIELD_NAME_REQUIRED, HP_MAX, HP_MIN, HP_REQUIRED, ID_INVALID, ID_NUMBER, INVALID_OTP, INVALID_OTP_LENGTH, INVALID_PHONE, INVALID_PHONE_NUMBER, INVALID_PHONE_NUMBER_VALID_LENGTH, INVALID_SIM, INVALID_SIM_NUMBER, INVALID_SIM_NUMBER_VALID_LENGTH, LOCATION_NAME_INVALID, LOCATION_NAME_MIN_LEN, MAC_MIN_LEN, MAC_REQUIRED, MIN_3_CHARACTERS_REQUIRED, MOTOR_MIN_LENGTH, MOTOR_NAME, MOTOR_NAME_STARTS_LETTER, MOTOR_REQUIRED, NAME_MIN_LENGTH, NAME_REQUIRED, OTP_REQUIRED, PASSWORD_MIN_LENGTH, PASSWORD_REQUIRED, PASSWORD_SHOULD_CONTAIN_NUMBER, PASSWORD_SHOULD_CONTAIN_UPPERCASE, PASSWORD_SPECIAL_CHAR, PCB_MIN_LEN, PCB_NUMBER_REQUIRED, PCB_SERIAL_NUMBER_REQUIRED, PHONE_NUMBER_REQUIRED, SERIAL_NO_MIN_LEN, SERIAL_NO_REQUIRED, SIM_NUMBER_REQUIRED, SMALL_LETTERS_NOT_ALLOWED, STARTER_NUMBER_MIN_LEN, STARTER_NUMBER_REQUIRED, TITLE_MUST_BE_STRING, TITLE_REQUIRED, USER_TYPE_INVALID, USER_TYPE_REQUIRED, VALID_MAIL, VALID_NAME } from "../../constants/app-constants.js";
import { userTypeEnum } from "../../constants/enum-types.js";
const phoneValidator = v.pipe(v.string(PHONE_NUMBER_REQUIRED), v.trim(), v.nonEmpty(PHONE_NUMBER_REQUIRED), v.regex(/^\d+$/, INVALID_PHONE_NUMBER), v.regex(/^[6-9]/, INVALID_PHONE), v.regex(/^\d{10}$/, INVALID_PHONE_NUMBER_VALID_LENGTH), v.regex(/^(?!(\d)\1{9}$).+$/, INVALID_PHONE));
const simNumberValidator = v.pipe(v.string(SIM_NUMBER_REQUIRED), v.trim(), v.nonEmpty(SIM_NUMBER_REQUIRED), v.regex(/^\d+$/, INVALID_SIM_NUMBER), v.regex(/^[6-9]/, INVALID_SIM), v.regex(/^\d{10}$/, INVALID_SIM_NUMBER_VALID_LENGTH), v.regex(/^(?!(\d)\1{9}$).+$/, INVALID_SIM));
const emailValidator = v.nullish(v.optional(v.pipe(v.string(EMAIL_REQUIRED), v.transform(value => value.trim()), v.nonEmpty(EMAIL_REQUIRED), v.email(VALID_MAIL))));
...
function requiredNumberOptional(errorMessage) {
    return v.optional(v.pipe(v.number(errorMessage), v.custom((value) => value !== 0, errorMessage)));
}
