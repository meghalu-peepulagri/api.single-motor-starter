...
const phoneValidator = v.pipe(v.string(PHONE_NUMBER_REQUIRED), v.transform(value => value.trim()), v.nonEmpty(PHONE_NUMBER_REQUIRED), v.regex(/^\d+$/, INVALID_PHONE_NUMBER), v.regex(/^[6-9]\d{9}$/, INVALID_PHONE_NUMBER));
...
const passwordValidator = v.pipe(v.string(PASSWORD_REQUIRED), v.transform(value => value.trim()), v.nonEmpty(PASSWORD_REQUIRED), v.minLength(6, PASSWORD_MIN_LENGTH));
...
export { emailValidator, nameValidator, passwordValidator, phoneValidator, requiredNumber, userTypeValidator };
