...
export const vAddMotorSchedule = v.pipe(v.object({
    ...
    starter_id: v.optional(v.pipe(v.number(STARTER_ID_REQUIRED), v.custom((value) => typeof value === "number" && Number.isInteger(value) && value > 0, "Invalid starter id"))),
    ...
    days_of_week: v.optional(v.pipe(v.array(v.number()), v.custom((val) => Array.isArray(val) && val.every((day) => DAYS_OF_WEEK_ENUM.includes(day)), INVALID_DAYS_WEEK))),
    // One-time schedule date (YYYY-MM-DD)
    schedule_date: v.optional(v.pipe(v.string(), v.regex(/^\d{4}-\d{2}-\d{2}$/, SCHEDULE_DATE_FORMAT))),
    ...
    cycle_on_minutes: v.optional(v.pipe(v.number(), v.custom((val) => typeof val === "number" && Number.isInteger(val) && val >= 1, CYCLE_OFF_MINUTES_MIN))),
    cycle_off_minutes: v.optional(v.pipe(v.number(), v.custom((val) => typeof val === "number" && Number.isInteger(val) && val >= 1, CYCLE_OFF_MINUTES_MIN))),
