...
export const vAddMotorSchedule = v.object({
    ...
    pond_id: v.pipe(v.number(FIELD_REQUIRED), v.custom((value) => typeof value === "number" && Number.isInteger(value) && value > 0, "Invalid pond id")),
    ...
    schedule_date: v.optional(v.pipe(v.string(SCHEDULE_DATE_REQUIRED), v.nonEmpty(SCHEDULE_DATE_REQUIRED), v.regex(/^\d{4}-\d{2}-\d{2}$/, SCHEDULE_DATE_FORMAT))),
