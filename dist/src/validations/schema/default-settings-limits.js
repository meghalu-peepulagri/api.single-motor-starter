 ...
 */
const realNonNegative = (fieldName) => v.pipe(v.number(`${fieldName} must be a number`), v.minValue(0, `${fieldName} cannot be negative`));
/**
 * Validation helper for integers with non-negative check
 */
const integerNonNegative = (fieldName) => v.pipe(v.number(`${fieldName} must be an integer`), v.integer(`${fieldName} must be an integer`), v.minValue(0, `${fieldName} cannot be negative`));
