 ...
 */
export const integerOnly = (field, options) => {
    ...
    return v.pipe(v.union([v.number(), v.string()], `${SETTINGS_FIELD_NAMES[field]} must be a valid number`), v.transform((val) => {
        ...
        return val;
    }), v.check((val) => typeof val === 'number' && !isNaN(val), `${SETTINGS_FIELD_NAMES[field]} must be a valid number, not NaN`), v.check((val) => isFinite(val), `${SETTINGS_FIELD_NAMES[field]} must be a finite number (not Infinity or -Infinity)`), v.check((val) => Number.isInteger(val), `${SETTINGS_FIELD_NAMES[field]} must be an integer without decimal places (decimal values not allowed)`), v.check((val) => val >= min, options?.allowNegative
        ? `${SETTINGS_FIELD_NAMES[field]} must be greater than or equal to ${min}`
        : `${SETTINGS_FIELD_NAMES[field]} must be a non-negative integer (negative values not allowed)`), v.check((val) => val <= max, `${SETTINGS_FIELD_NAMES[field]} must be less than or equal to ${max}`));
};
/**
 ...
 * - Rejects NaN, Infinity, -Infinity
 */
export const realOnly = (field, options) => {
    ...
    return v.pipe(v.union([v.number(), v.string()], `${SETTINGS_FIELD_NAMES[field]} must be a valid number`), v.transform((val) => {
        ...
        return val;
    }), v.check((val) => typeof val === 'number' && !isNaN(val), `${SETTINGS_FIELD_NAMES[field]} must be a valid number, not NaN`), v.check((val) => isFinite(val), `${SETTINGS_FIELD_NAMES[field]} must be a finite number (not Infinity or -Infinity)`), v.check((val) => val >= min, options?.allowNegative
        ? `${SETTINGS_FIELD_NAMES[field]} must be greater than or equal to ${min}`
        : `${SETTINGS_FIELD_NAMES[field]} must be a non-negative number (negative values not allowed)`), v.check((val) => val <= max, `${SETTINGS_FIELD_NAMES[field]} must be less than or equal to ${max}`), v.check((val) => {
        const decimalCount = countDecimalPlaces(val);
        return decimalCount <= decimalPlaces;
    }, `${SETTINGS_FIELD_NAMES[field]} can have maximum ${decimalPlaces} decimal place${decimalPlaces > 1 ? 's' : ''} (too many decimal places provided)`), v.transform((val) => {
 ...
 * - Rejects strings "0"/"1"
 */
export const enable01 = (field) => v.pipe(v.union([v.number(), v.string()], `${SETTINGS_FIELD_NAMES[field]} must be 0 or 1`), v.transform((val) => {
    // Handle string to number conversion
    if (typeof val === 'string') {
        const trimmed = val.trim();
    ...
    return v.pipe(v.union([v.array(v.union([v.string(), v.number()])), v.null(), v.undefined()], `${SETTINGS_FIELD_NAMES[field]} must be an array of mobile numbers`), v.transform((val) => {
        ...
        return arr.map((item) => {
            ...
            if (typeof item === 'number') {
                ...
                throw new Error(`${SETTINGS_FIELD_NAMES[field]} contains invalid item type: ${typeof item}`);
            }
...
const waitForAck = (identifiers, timeout, sequenceNumber) => {
