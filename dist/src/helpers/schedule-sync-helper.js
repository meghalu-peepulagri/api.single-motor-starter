...
export async function pushPendingSchedulesForStarter(starter, motorId, filterIds, fromHeartbeat = false) {
    ...
    try {
            ...
            await db.transaction(async (trx) => {
                for (const [motorRowId, motorUnassigned] of unassignedByMotor) {
                    ...
                    const occupiedRows = await trx
                        ...
                        .from(motorSchedules)
                        .where(and(eq(motorSchedules.motor_id, motorRowId), ne(motorSchedules.status, "ARCHIVED"), notInArray(motorSchedules.schedule_status, [...SLOT_FREE_STATUSES]), isNotNull(motorSchedules.device_schedule_id)));
        ...
        const [ackedRow] = await db
            ...
            .from(motorSchedules)
            .where(and(eq(motorSchedules.starter_id, starter.id), eq(motorSchedules.acknowledgement, 1), ne(motorSchedules.status, "ARCHIVED"), notInArray(motorSchedules.schedule_status, [...SLOT_FREE_STATUSES]), gte(motorSchedules.schedule_end_date, todayNum)))
            .limit(1);
 ...
 */
export async function triggerSyncForCreatedSchedules(records) {
    const today = todayAsYYMMDD();
