    ...
    getUnifiedLogsHandler = async (c) => {
        try {
            ...
            const [data, totalRecords, starterBox] = await Promise.all([
                ...
                getRecordById(starterBoxes, starterId, ["motor_support_type"]),
            ]);
            const paginationInfo = getPaginationData(page, pageSize, totalRecords);
            ...
            if (starterBox?.motor_support_type === "MULTIPLE_MOTORS") {
                const motor = await getRecordById(motors, motorId, ["alias_name", "name"]);
                const motorName = motor?.alias_name || motor?.name || null;
                records = records.map((record) => record.entity_type === "MOTOR" ? { ...record, motor_name: motorName } : record);
            }
            const response = {
                pagination: paginationInfo,
                records,
