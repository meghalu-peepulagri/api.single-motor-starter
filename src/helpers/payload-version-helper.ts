...
type VersionedStarter = {
  payload_version?: string | null;
  motor_support_type?: string | null;
  starter_type?: string | null;
};

/** Anything unrecognised resolves to "1.0" — the safe direction for an unknown board. */
export function payloadVersionOf(starter: VersionedStarter | null | undefined): PayloadVersion {
  return starter?.payload_version === "2.0" ? "2.0" : "1.0";
}

export function isV2Payload(starter: VersionedStarter | null | undefined): boolean {
 ...
 * motor_support_type.
 */
export function isDualMotor(starter: VersionedStarter | null | undefined): boolean {
 ...
 */
export function isInvalidVersionMotorPair(version: PayloadVersion, dualMotor: boolean): boolean {
