 ...
 */
function cyclicPlannedOnMinutes(
  schedule: ScheduleForEvaluation,
  startMinutes: number,
  endMinutes: number,
): number {
  ...
  if (on <= 0) return span;               // no usable cyclic info → fall back to span
  const cycle = on + off;
  if (cycle <= 0) return on;              // off missing → at least one ON period
