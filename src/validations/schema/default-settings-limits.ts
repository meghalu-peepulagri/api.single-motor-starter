 ...
 */
const realLimitValue = (fieldName: string, options?: { decimalPlaces?: number }) => {
  const dp = options?.decimalPlaces ?? 2;
  const factor = Math.pow(10, dp);
  return v.pipe(
    v.union([v.number(), v.string()], `${fieldName} must be a valid number`),
    v.transform((val) => {
    ...
    v.check(
      ...
      `${fieldName} must be a valid number, not NaN`
    ),
    v.check(
      (val) => isFinite(val),
      `${fieldName} must be a finite number (not Infinity or -Infinity)`
    ),
    v.check(
      (val) => val >= 0,
      `${fieldName} cannot be negative (negative values not allowed)`
    ),
    v.check(
      (val) => {
      ...
      `${fieldName} can have maximum ${dp} decimal places`
    ),
    v.transform((val) => {
 ...
 */
const integerLimitValue = (fieldName: string) =>
  v.pipe(
    v.union([v.number(), v.string()], `${fieldName} must be a valid integer`),
    v.transform((val) => {
      if (typeof val === 'string') {
        const trimmed = val.trim();
        if (trimmed === '') {
    ...
    v.check(
      ...
      `${fieldName} must be an integer without decimal places`
    ),
    v.check(
      (val) => val >= 0,
      `${fieldName} cannot be negative (negative values not allowed)`
 ...
 */
const minMaxPairs: Array<[string, string, string]> = [
 ...
 */
export const vUpdateDefaultSettingsLimits = v.pipe(
  partialSchema,
  v.check((data) => {
    // Validate all min/max pairs
    for (const [minField, maxField, displayName] of minMaxPairs) {
      const minValue = (data as any)[minField];
      const maxValue = (data as any)[maxField];

      // Only validate if both values are present
      if (minValue !== undefined && maxValue !== undefined) {
        if (maxValue < minValue) {
          throw new Error(
            `${displayName}: maximum value (${maxValue}) cannot be less than minimum value (${minValue})`
          );
        }
      }
    }
