...
const phoneValidator = v.pipe(
  ...
  v.regex(/^[6-9]/, INVALID_PHONE),
  v.regex(/^\d{10}$/, INVALID_PHONE_NUMBER_VALID_LENGTH),
  v.regex(/^(?!(\d)\1{9}$).+$/, INVALID_PHONE),
);

const simNumberValidator = v.pipe(
  v.string(SIM_NUMBER_REQUIRED),
  v.trim(),
...
export {
  ...
  starterNumberValidator, pcbOrSerialNumberValidator, hardwareVersion, simNumberValidator
};
