...
const requiredStr = (msg: string) => v.pipe(
  v.string(msg),
  v.trim(),
  v.nonEmpty(msg),
);
...
const requiredStrMin = (msg: string, minLen: number, minMsg: string) => v.pipe(
  v.string(msg),
  v.trim(),
  v.nonEmpty(msg),
  v.minLength(minLen, minMsg),
);

// Contact number validator
const contactNumberValidator = v.pipe(
  v.string(CONTACT_NUMBER_REQUIRED),
  v.trim(),
  v.nonEmpty(CONTACT_NUMBER_REQUIRED),
  v.regex(/^\d+$/, CONTACT_NUMBER_INVALID),
  v.regex(/^\d{10}$/, CONTACT_NUMBER_VALID_LENGTH),
);

// Payment status validator
const paymentStatusValidator = v.pipe(
  v.string(PAYMENT_STATUS_REQUIRED),
  v.trim(),
  v.nonEmpty(PAYMENT_STATUS_REQUIRED),
  v.picklist(paymentStatusEnum.enumValues, INVALID_PAYMENT_STATUS),
);
...
export type ValidatedAddStarterDispatch = v.InferOutput<typeof vAddStarterDispatch>;
