...
export class StarterHandlers {
  ...
  replaceStarterLocationHandler = async (c: Context) => {
    try {
      ...
      const [oldLocation, newLocation] = await Promise.all([
        starter.location_id
          ? getSingleRecordByMultipleColumnValues(locations, ["id"], ["="], [starter.location_id], ["id", "name"] as any)
          : Promise.resolve(null),
        getSingleRecordByMultipleColumnValues(locations, ["id"], ["="], [validatedStarterReq.location_id], ["id", "name"] as any),
      ]);
      ...
      await db.transaction(async (trx) => {
        ...
        const oldLocName = (oldLocation as any)?.name ?? `location ${starter.location_id}`;
        const newLocName = (newLocation as any)?.name ?? `location ${validatedStarterReq.location_id}`;
  ...
  assignStarterWebHandler = async (c: Context) => {
    try {
      ...
      const assignedUser = await getSingleRecordByMultipleColumnValues<UsersTable>(
        users, ["id", "status"], ["=", "!="], [validatedReqData.user_id, "ARCHIVED"], ["id", "full_name"] as any
      );
      ...
      await db.transaction(async (trx) => {
        ...
        await ActivityService.writeStarterAssignedLog(userPayload.id, (starterBox as any).id, { user_id: updatedStarter.user_id }, trx, webAssignMsg);
      });
