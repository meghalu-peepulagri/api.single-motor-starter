...
import type { validatedAddStarter, validatedAssignLocationToStarter, validatedAssignStarter, validatedAssignStarterWeb, validatedReplaceStarter, ValidatedReplaceBox, ValidatedReplacePcb, validatedUpdateDeployedStatus, ValidatedUpdateStarterDetails } from "../validations/schema/starter-validations.js";
...
export class StarterHandlers {
  ...
  getConsecutiveAlertsFaultsHandler = async (c: Context) => {
    try {
      ...
      const totalRecords = await getConsecutiveGroupsCount(starterId, motorId, type as 'alert' | 'fault', assignedAtDate);
  ...
  getUnifiedLogsHandler = async (c: Context) => {
    try {
      ...
      const logTypes = query.log_type
        ? query.log_type.split(",").map((t: string) => t.trim().toLowerCase())
        : null;
      const { page, pageSize, offset } = getPaginationOffParams(query);
      const assignedAt = query.is_assigned === "true" ? await getSingleRecordByMultipleColumnValues<StarterBoxTable>(starterBoxes, ["id", "status"], ["=", "!="], [starterId, "ARCHIVED"], ["assigned_at"]) : null;
      const assignedAtDate = assignedAt?.assigned_at ?? null;
    ...
    try {
      ...
      if (motorCount > 0 && validatedReqData.motors.length !== motorCount) {
        throw new BadRequestException(
          motorCount === 1 ? ASSIGN_SINGLE_MOTOR_DEVICE_ONLY_ONE : ASSIGN_DUAL_MOTOR_DEVICE_REQUIRES_BOTH
        );
      }
  ...
  updateDeployStatusHandler = async (c: Context) => {
    try {
      ...
      const starterBox = await getSingleRecordByMultipleColumnValues<StarterBoxTable>(starterBoxes, ["id", "status"], ["=", "!="], [starterId, "ARCHIVED"], ["id", "device_status", "status", "device_allocation"]);
      if (!starterBox) throw new BadRequestException(STARTER_BOX_NOT_FOUND);
  ...
  replaceBoxHandler = async (c: Context) => {
    try {
      const userPayload: User = c.get("user_payload");
      const starterId = +(c.req.param("id") ?? 0);
      const reqData = await c.req.json();
  ...
  replacePcbHandler = async (c: Context) => {
    try {
      ...
      const validatedReqData = await validatedRequest<ValidatedReplacePcb>("replace-pcb", reqData, REPLACE_DEVICE_VALIDATION_CRITERIA);
  ...
  starterCountBasedOnStatusHandler = async (c: Context) => {
    try {
      ...
      const filtersFor = (deviceStatus: "READY" | "TEST" | "DEPLOYED" | "ASSIGNED" | "REPLACED") =>
        starterCountFilters({ ...query, device_status: deviceStatus });
  ...
  simRechargeExpiryNotificationHandler = async (c: Context) => {
    try {
  ...
  deviceInfoRequestHandler = async (c: Context) => {
    try {
  ...
  getDeviceDetailsHandler = async (c: Context) => {
    try {
      const body = await c.req.json();
