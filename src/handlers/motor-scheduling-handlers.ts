...
const waitForPublishSlot = async (starterId: number, maxWaitMs = 15000, intervalMs = 300) => {
...
export class MotorScheduleHandler {
      ...
      if (toEvaluate.length > 0) {
        ...
        if (transitions.length > 0) {
          ...
          result.records = result.records.map(r =>
            statusMap.has(r.id) ? { ...r, schedule_status: statusMap.get(r.id) as any } : r
          );
        }
      }
  ...
  getMotorScheduleByIdHandler = async (c: Context) => {
    try {
      ...
      let schedule = await getRecordById<MotorScheduleTable>(motorSchedules, scheduleId);
      ...
      if (toEvaluate.length > 0) {
        const transitions = await runScheduleEvaluator(toEvaluate).catch(() => []);
        if (transitions.length > 0) {
          schedule = { ...schedule, schedule_status: transitions[0].to as any };
        }
      }
  ...
  deleteMotorScheduleHandler = async (c: Context) => {
    try {
      ...
      ]) as Pick<MotorSchedule, "id" | "schedule_status" | "acknowledgement" | "start_time" | "end_time" | "schedule_start_date" | "starter_id" | "motor_id"> | null;
      if (!existed) throw new BadRequestException(SCHEDULE_NOT_FOUND);
  ...
  bulkUpdateAcknowledgementHandler = async (c: Context) => {
    try {
      ...
      if (slotMap && Object.keys(slotMap).length > 0) {
        ...
        const byMotor = new Map<number, number[]>();
        for (const row of rows) {
          if (!row.motor_id) continue;
        ...
        const byMotor = new Map<number, { id: number; schedule_id: number }[]>();
        for (const row of rows) {
          if (!row.motor_id) continue;
          const list = byMotor.get(row.motor_id) ?? [];
  ...
  bulkDeleteSchedulesHandler = async (c: Context) => {
    try {
      ...
      const motorIds = [...new Set(toDelete.map(r => r.motor_id).filter((id): id is number => id != null))];
  ...
  getPendingSchedulesForSyncHandler = async (c: Context) => {
    try {
      ...
      for (const starterId of starterIds) {
        ...
        const byMotor = new Map<number, typeof starterRecords>();
        ...
        for (const [motorId, motorRecords] of byMotor) {
          ...
          for (const r of motorRecords) {
            (r as any).device_schedule_id = idToSlot.get(r.id) ?? (r as any).device_schedule_id;
          }
          logger.info(`[sync/pending] starter=${starterId} motor=${motorId} assigned device_schedule_ids=[${motorRecords.map(r => (r as any).device_schedule_id).join(",")}]`);
        }
      }
  ...
  republishSchedulesHandler = async (c: Context) => {
    try {
      ...
      const starterMstA = await getRecordById(starterBoxes, starterId, ["motor_support_type", "payload_version"]) as { motor_support_type: string | null; payload_version: string | null } | null;
      const singleMotorStarterIds = !isV2Payload(starterMstA) && starterMstA?.motor_support_type === "SINGLE_MOTOR" ? new Set([starterId]) : new Set<number>();
      const grouped = buildDeviceSyncPayloads(records, firstSyncStarterIds, singleMotorStarterIds);
    ...
    try {
      ...
      await db.update(motorSchedules)
        ...
        .where(and(
          ...
          inArray(motorSchedules.schedule_status, ["FAILED", "PENDING", "SCHEDULED"]),
          ne(motorSchedules.status, "ARCHIVED"),
          ids && ids.length > 0 ? inArray(motorSchedules.id, ids) : undefined,
        ));
      ...
      // Only V1.0 single-motor boxes get the legacy flat `m1: [...]` array; a V2.0 box uses
      // the per-motor object form produced by the multi-motor branch. See schedule-sync-helper.
      const starterMstB = await getRecordById(starterBoxes, starterId, ["motor_support_type", "payload_version"]) as { motor_support_type: string | null; payload_version: string | null } | null;
      const singleMotorStarterIds = !isV2Payload(starterMstB) && starterMstB?.motor_support_type === "SINGLE_MOTOR" ? new Set([starterId]) : new Set<number>();
      const grouped = buildDeviceSyncPayloads(records, firstSyncStarterIds, singleMotorStarterIds);
      ...
      for (const { chunks } of grouped) {
        for (const { payload, dbIds, scheduleIds, motorRefs } of chunks) {
          ...
          // Don't let a mid-flight heartbeat publish cause this resync to be skipped.
          await waitForPublishSlot(starterId);
          const ok = await publishMultipleTimesInBackground(payload, starter as any);
          if (ok) {
            published++;
            const partialIds = publishKey ? schedulePartialAckMap.get(publishKey) : undefined;
            if (publishKey) schedulePartialAckMap.delete(publishKey);
