...
import type { ValidatedAddGateway, ValidatedAssignGatewayToUser, ValidatedRenameGateway, ValidatedUpdateGatewayDetails, ValidatedUpdateGatewayLabel, ValidatedUpdateGatewayNumber } from "../validations/schema/gateway-validations.js";
...
export class GatewayHandlers {
  ...
  assignGatewayToUserHandler = async (c: Context) => {
    try {
      const userPayload = c.get("user_payload");
      const reqBody = await c.req.json();
      const validReq = await validatedRequest<ValidatedAssignGatewayToUser>(
        "assign-gateway",
        reqBody,
      ...
      if (validReq.user_id && !isAdmin) throw new ForbiddenException();
      ...
      const result = await db.transaction(async (tx) => {
        ...
        if (!assignment) throw new NotFoundException(GATEWAY_NOT_FOUND);
  ...
  listGatewaysHandler = async (c: Context) => {
    try {
      const userPayload = c.get("user_payload");
  ...
  deleteGatewayHandler = async (c: Context) => {
    try {
      ...
      await db.transaction(async (tx) => {
        ...
        await updateRecordById<GatewayTable>(gateways, foundGateway.id, { status: "ARCHIVED", user_id: null }, tx);
  ...
  updateGatewayLabelHandler = async (c: Context) => {
    try {
      ...
      const validReq = await validatedRequest<ValidatedUpdateGatewayLabel>(
      ...
      await db.transaction(async (tx) => {
        await updateRecordById<GatewayTable>(gateways, gatewayId, { label: validReq.label }, tx);
  ...
  renameGatewayHandler = async (c: Context) => {
    try {
      ...
      const reqBody = await c.req.json();
      paramsValidateException.emptyBodyValidation(reqBody);
      const validReq = await validatedRequest<ValidatedRenameGateway>(
        "rename-gateway",
        reqBody,
      ...
      await db.transaction(async (tx) => {
        await updateRecordById<GatewayTable>(gateways, gatewayId, { name: validReq.name }, tx);
  ...
  updateGatewayNumberHandler = async (c: Context) => {
    try {
      ...
      const reqBody = await c.req.json();
      paramsValidateException.emptyBodyValidation(reqBody);
      const validReq = await validatedRequest<ValidatedUpdateGatewayNumber>(
        "update-gateway-number",
        reqBody,
      ...
      await db.transaction(async (tx) => {
        await updateRecordById<GatewayTable>(gateways, gatewayId, { gateway_number: validReq.gateway_number }, tx);
