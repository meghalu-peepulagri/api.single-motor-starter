...
export class StarterDefaultSettingsHandlers {
  ...
  updateLatestSettingAckByStarterHandler = async (c: Context) => {
    try {
      const userPayload = c.get("user_payload");
      const starterId = +(c.req.param("starter_id") ?? 0);
      const starterData = await getSingleRecordByMultipleColumnValues<StarterBoxTable>(starterBoxes, ["id", "status"], ["=", "!="], [starterId, "ARCHIVED"], ["id", "pcb_number", "synced_settings_status"]);
      if (!starterData) throw new BadRequestException(DEVICE_NOT_FOUND);
      ...
      if (starterData.synced_settings_status === "false") {
        await updateRecordById<StarterBoxTable>(starterBoxes, starterData.id, { synced_settings_status: "true" });
      }
      await ActivityService.logActivity({
