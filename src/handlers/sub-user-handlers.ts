...
} from "../validations/schema/sub-user-validations.js";
...
export class SubUserHandlers {
  ...
  createSubUserHandler = async (c: Context) => {
    try {
      ...
      if (error instanceof v.ValiError) throw new BadRequestException(error.message);
  ...
  assignPermissionsHandler = async (c: Context) => {
    try {
      const user = c.get("user_payload");
      const subId = Number(c.req.param("id"));
      paramsValidateException.validateId(subId, "sub-user id");
