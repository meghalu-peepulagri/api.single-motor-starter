...
export class MotorHandlers {
  ...
  getAllMotorsHandler = async (c: Context) => {
    try {
      ...
      const orderQueryData = parseOrderByQueryCondition<MotorsTable>(query.order_by, query.order_type, isAdmin ? "created_at" : "assigned_at", "desc");
  ...
  assignUserToMotorHandler = async (c: Context) => {
    try {
      const userPayload = c.get("user_payload");
      const motorId = +c.req.param("id");
      paramsValidateException.validateId(motorId, "motor id");
      ...
      if (!user_id || typeof user_id !== "number") throw new BadRequestException("user_id is required and must be a number");

      const motor = await getSingleRecordByMultipleColumnValues<MotorsTable>(motors, ["id", "status"], ["=", "!="], [motorId, "ARCHIVED"]);
      if (!motor) throw new NotFoundException(MOTOR_NOT_FOUND);

      const user = await getSingleRecordByMultipleColumnValues<UsersTable>(users, ["id", "status"], ["=", "!="], [user_id, "ARCHIVED"]);
      ...
      await db.transaction(async (trx) => {
        if (motor.starter_id) {
          ...
          await updateRecordById<StarterBoxTable>(starterBoxes, motor.starter_id, { user_id }, trx);
        } else {
          await updateRecordById<MotorsTable>(motors, motorId, { user_id }, trx);
        }
