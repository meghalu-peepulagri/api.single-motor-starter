...
import type { validatedAddMotor, validatedAddMotorToStarter, validatedControlMotors, validatedControlMotorsMode, validatedUpdateMotor, validatedUpdateMotorTestRunStatus } from "../validations/schema/motor-validations.js";
...
export class MotorHandlers {
   ...
   */
  addMotorToStarterHandler = async (c: Context) => {
    try {
      ...
      const motorPayload = await c.req.json();
      paramsValidateException.emptyBodyValidation(motorPayload);
      const validMotorReq = await validatedRequest<validatedAddMotorToStarter>("add-motor-to-starter", motorPayload, MOTOR_VALIDATION_CRITERIA);
      ...
      if (!isV2Payload(starter)) throw new BadRequestException(DUAL_MOTOR_CONVERSION_REQUIRES_V2);
      if (isDualMotor(starter)) throw new BadRequestException(DUAL_MOTOR_CONVERSION_ALREADY_DUAL);
      ...
      if (liveMotors.length >= 2) throw new BadRequestException(DUAL_MOTOR_CONVERSION_ALREADY_DUAL);
      if (!existingMotor) throw new BadRequestException(DUAL_MOTOR_CONVERSION_NO_EXISTING_MOTOR);
  ...
  controlMotorsHandler = async (c: Context) => {
    try {
      const starterId = +(c.req.param("starterId") ?? 0);
      ...
      const validReq = await validatedRequest<validatedControlMotors>("control-motors", requestBody, MOTOR_CONTROL_VALIDATION_CRITERIA);
  ...
  controlMotorsModeHandler = async (c: Context) => {
    try {
      const starterId = +(c.req.param("starterId") ?? 0);
      paramsValidateException.validateId(starterId, "starter id");
      ...
      const requestBody = await c.req.json();
      paramsValidateException.emptyBodyValidation(requestBody);
      const validReq = await validatedRequest<validatedControlMotorsMode>("control-motors-mode", requestBody, MOTOR_MODE_CONTROL_VALIDATION_CRITERIA);
