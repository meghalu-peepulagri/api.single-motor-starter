...
export async function heartbeatHandler(message: any, topic: string) {
  try {
    ...
    if (device.signal_quality !== strength || device.network_type !== message.D.nwt) await updateRecordById<StarterBoxTable>(starterBoxes, device.id, { signal_quality: strength, network_type: validNetwork, status: status });
