...
export async function getStarterRunTime(
  ...
  const totalOnSeconds = splitRecords.reduce((sum, record) => {
    if ((!powerState && record.power_state !== 1) || !record.duration) return sum;
...
// =================== TOPOLOGY (role / parent linkage) ===================

type DeviceRole = "STANDALONE" | "MASTER" | "CHILD";

export async function getStarterById(id: number) {
  return await getSingleRecordByMultipleColumnValues<StarterBoxTable>(
    starterBoxes, ["id", "status"], ["=", "!="], [id, "ARCHIVED"]
  );
}
 ...
 */
export async function resolveAndValidateParent(
  role: DeviceRole,
  parentStarterId: number | null | undefined,
  selfId?: number,
): Promise<{ parent: StarterBox | null }> {
  if (role !== "CHILD") {
    return { parent: null };
  }
  if (!parentStarterId) {
    throw new (await import("../../exceptions/bad-request-exception.js")).default(
      (await import("../../constants/app-constants.js")).PARENT_REQUIRED_FOR_CHILD,
    );
  }
  if (selfId && parentStarterId === selfId) {
    throw new (await import("../../exceptions/bad-request-exception.js")).default(
      (await import("../../constants/app-constants.js")).CANNOT_PARENT_SELF,
    );
  }
  const parent = await getStarterById(parentStarterId);
  if (!parent) {
    throw new (await import("../../exceptions/not-found-exception.js")).default(
      (await import("../../constants/app-constants.js")).PARENT_NOT_FOUND,
    );
  }
  if (parent.role !== "MASTER") {
    throw new (await import("../../exceptions/bad-request-exception.js")).default(
      (await import("../../constants/app-constants.js")).PARENT_MUST_BE_MASTER,
    );
  }
  return { parent };
}

export async function getChildrenOfStarter(masterId: number) {
...
export async function countChildrenOfMaster(masterId: number): Promise<number> {
 ...
 */
export async function findMasterByIdentifier(identifier: string) {
  if (!identifier) return null;
  const upper = identifier.trim().toUpperCase();
  return await db.query.starterBoxes.findFirst({
    where: and(
      or(eq(starterBoxes.mac_address, upper), eq(starterBoxes.pcb_number, upper)),
 ...
 */
export async function findChildOfMasterByIdentifier(masterId: number, identifier: string) {
  if (!identifier || !masterId) return null;
  const upper = identifier.trim().toUpperCase();
  return await db.query.starterBoxes.findFirst({
    where: and(
      or(eq(starterBoxes.mac_address, upper), eq(starterBoxes.pcb_number, upper)),
      eq(starterBoxes.role, "CHILD"),
      eq(starterBoxes.parent_starter_id, masterId),
      ne(starterBoxes.status, "ARCHIVED"),
    ),
  });
...
export async function getMasterIdentifierById(masterId: number): Promise<string | null> {
 ...
 */
export async function getTopologyTree(query: { user_id?: number; location_id?: number; status?: string }) {
  const filters: any[] = [ne(starterBoxes.status, "ARCHIVED")];
  if (query.user_id) filters.push(eq(starterBoxes.user_id, Number(query.user_id)));
  if (query.location_id) filters.push(eq(starterBoxes.location_id, Number(query.location_id)));
  if (query.status) filters.push(eq(starterBoxes.status, query.status as any));
...
export async function changeRoleWithTransaction(
  starter: StarterBox,
  payload: {
    role: DeviceRole;
    parent_starter_id?: number | null;
    reassignment?: "ORPHAN" | "REPARENT";
    ...
    if (childCount > 0) {
      ...
      if (strategy === "REPARENT") {
        if (!payload.new_parent_id) {
          throw new (await import("../../exceptions/bad-request-exception.js")).default(
            (await import("../../constants/app-constants.js")).NEW_PARENT_REQUIRED,
          );
        }
  ...
  return await db.transaction(async (trx) => {
    if (oldRole === "MASTER" && newRole !== "MASTER" && childrenReassigned > 0) {
      ...
      if (strategy === "REPARENT") {
        await trx.update(starterBoxes)
          ...
          .where(and(
            eq(starterBoxes.parent_starter_id, starter.id),
            ne(starterBoxes.status, "ARCHIVED"),
          ));
      } else {
        await trx.update(starterBoxes)
    ...
    const updated = await updateRecordByIdWithTrx<StarterBoxTable>(starterBoxes, starter.id, updateData, trx);
...
export async function reparentWithTransaction(
  child: StarterBox,
  newParentId: number,
  userId: number,
) {
  if (child.role !== "CHILD") {
    throw new (await import("../../exceptions/bad-request-exception.js")).default(
      (await import("../../constants/app-constants.js")).DEVICE_IS_NOT_CHILD,
    );
  }
  ...
  return await db.transaction(async (trx) => {
    ...
    const updated = await updateRecordByIdWithTrx<StarterBoxTable>(
      starterBoxes, child.id, { parent_starter_id: newParentId }, trx
    );
 ...
 */
export async function getStarterTopologyContext(starterId: number) {
