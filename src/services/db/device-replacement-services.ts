...
import type { MotorSettingsBlock } from "../../types/multi-motor-settings-types.js";
 ...
 * new one instead.
 */
async function lockReplaceableStarter(trx: any, starterId: number): Promise<StarterBox> {
 ...
 */
async function lockSpareDeviceByPcb(trx: any, oldId: number, newPcbNumber: string): Promise<StarterBox | undefined> {
 ...
 */
async function ensureSpareIsDeployable(trx: any, old: StarterBox, spare: StarterBox) {
  ...
  if (spare.device_status !== "DEPLOYED") {
    throw new ReplacementAttemptError(
      "FAILED",
      `Device ${spare.starter_number} is in ${spare.device_status} state, not DEPLOYED`,
      new ConflictException(REPLACE_SPARE_DEVICE_NOT_DEPLOYED, { field: "pcb_number" }),
      old,
    );
  }
...
async function deviceMotorArity(trx: any, device: StarterBox): Promise<"SINGLE_MOTOR" | "MULTIPLE_MOTORS"> {
  const [row] = await trx.select({ count: sql<number>`count(*)` }).from(motors)
 ...
 */
async function retireDeviceAndMotors(trx: any, device: StarterBox) {
  await trx.update(motors).set({ status: "ARCHIVED" })
    .where(and(eq(motors.starter_id, device.id), ne(motors.status, "ARCHIVED")));
  await updateRecordById<StarterBoxTable>(starterBoxes, device.id, { status: "ARCHIVED", device_status: "REPLACED" }, trx);
}
...
const BOARD_MOTOR_BLOCK_FIELDS = ["ig_r", "ig_y", "ig_b", "io_r", "io_y", "io_b"] as const;

/** The configuration a device is meant to be running — the same row the settings screen and publish path read. */
async function currentSettings(trx: any, starterId: number): Promise<StarterSettings | undefined> {
...
async function handOverSettings(
  trx: any,
  params: {
    customerStarterId: number;
    boardStarterId: number;
    targetStarterId: number;
    oldMotorIndexById: Map<number, number>;
    motorIdByIndex: Map<number, number>;
    performerId: number;
  },
): Promise<boolean> {
  ...
  const board = (await currentSettings(trx, params.boardStarterId))
    ?? ((await getStarterDefaultSettings())[0] as unknown as StarterSettings | undefined);
  ...
  for (const field of BOARD_SETTINGS_FIELDS) {
    const value = (board as unknown as Record<string, unknown> | undefined)?.[field];
    if (value !== undefined) boardFields[field] = value;
  }
  ...
  if (multiMotorConfig) {
    ...
    for (const block of multiMotorConfig.motors) {
      ...
      if (targetMotorId === undefined) continue;
      const boardBlock = boardBlocks.get(motorIndex);
      const boardCalibration: Partial<MotorSettingsBlock> = {};
      for (const field of BOARD_MOTOR_BLOCK_FIELDS) {
        if (boardBlock?.[field] !== undefined) boardCalibration[field] = boardBlock[field];
      }
      motorBlocks.push({
        ...block,
        ...boardCalibration,
  ...
  await updateRecordById<StarterBoxTable>(starterBoxes, params.targetStarterId, { synced_settings_status: "false" }, trx);
  return true;
}
 ...
 */
const UPCOMING_SCHEDULE_STATUSES = ["PENDING", "SCHEDULED", "RUNNING", "WAITING_NEXT_CYCLE", "RESTARTED", "UNDELIVERED"] as const;
...
const resendScheduleFields = () => ({
  schedule_status: "PENDING" as const,
  acknowledgement: 0,
...
const upcomingSchedulesOf = (motorId: number) => and(
  eq(motorSchedules.motor_id, motorId),
  ne(motorSchedules.status, "ARCHIVED"),
  inArray(motorSchedules.schedule_status, [...UPCOMING_SCHEDULE_STATUSES]),
  gte(motorSchedules.schedule_end_date, todayAsYYMMDD()),
);
 ...
 */
async function handOverSchedules(trx: any, spareId: number, oldMotorIds: Map<number, number>, spareMotorIdByIndex: Map<number, number>, performerId: number) {
  await trx.update(motorSchedules).set({ schedule_status: "DELETED", deleted_by: performerId, deleted_at: new Date(), updated_at: new Date() })
    .where(and(
      eq(motorSchedules.starter_id, spareId),
      ne(motorSchedules.status, "ARCHIVED"),
  ...
  for (const [oldMotorId, motorIndex] of oldMotorIds) {
    ...
    if (spareMotorId === undefined) continue;
...
async function resendSchedules(trx: any, motorIds: number[]) {
...
export async function replacePcbWithTransaction(input: ReplaceDeviceInput): Promise<StarterBox> {
  const device = await runLoggedReplacement("PCB", input, async (trx, remember) => {
    ...
    const updated = await updateRecordById<StarterBoxTable>(starterBoxes, starter.id, {
    ...
    const liveMotors: Pick<Motor, "id" | "motor_index">[] = await trx.select({ id: motors.id, motor_index: motors.motor_index }).from(motors)
...
export async function replaceBoxWithTransaction(input: ReplaceDeviceInput & { newStarterNumber: string }): Promise<{ device: StarterBox; replacedDeviceId: number }> {
  const result = await runLoggedReplacement("BOX", input, async (trx, remember) => {
    ...
    if (spare.starter_number.toLowerCase() !== input.newStarterNumber.toLowerCase()) {
      throw new ReplacementAttemptError(
        "FAILED",
        `PCB Number ${input.newPcbNumber} belongs to device ${spare.starter_number}, not ${input.newStarterNumber}`,
        new UnprocessableEntityException(REPLACE_SPARE_NUMBERS_MISMATCH, { new_starter_number: REPLACE_SPARE_NUMBERS_MISMATCH }),
        old,
      );
    }
    await ensureSpareIsDeployable(trx, old, spare);

    const oldMotors: Motor[] = await trx.select().from(motors)
      .where(and(eq(motors.starter_id, old.id), ne(motors.status, "ARCHIVED")));
    const spareMotors: Pick<Motor, "id" | "motor_index">[] = await trx.select({ id: motors.id, motor_index: motors.motor_index }).from(motors)
    ...
    const device = await updateRecordById<StarterBoxTable>(starterBoxes, spare.id, {
      name: old.name,
 ...
 */
function armResyncAfterReplacement(starterId: number) {
