...
export async function paginatedMotorsList(whereQueryData: WhereQueryData<MotorsTable>, orderByQueryData: OrderByQueryData<MotorsTable>,
  ...
  const whereConditions = [...(prepareWhereQueryConditions<MotorsTable>(motors, whereQueryData) ?? []), ...extraConditions];
 ...
 */
export async function getMotorsLatestRuntime(
  motorIds: number[]
): Promise<Record<number, { state: string; duration: string }>> {
  ...
  const map: Record<number, { state: string; duration: string }> = {};

  // ✅ Handle motors WITH records
  for (const row of rows) {
    if (!row.motor_id || !row.start_time) continue;
...
export async function getMotorsTotalRunOnTime(motorIds: number[]) {
  ...
  type RuntimeRow = { motor_id: number | null; start_time: Date; end_time: Date | null; motor_state: number | null; offline_at?: Date | null };
  const allRecords: RuntimeRow[] = [...completedRecords, ...openRecords];

  // Group by motor_id
  const groupedByMotor: Record<number, RuntimeRow[]> = {};
  ...
  const action = async (trx: DbTransaction) => {
    ...
    if (inactiveIds.length > 0) {
      ...
      const openMotorRuntimeRows = await trx
        ...
        .where(
          and(
            inArray(motorsRunTime.starter_box_id, inactiveIds),
            isNull(motorsRunTime.end_time)
          )
        );
      ...
      const openDeviceRuntimeRows = await trx
        ...
        .where(
          and(
            inArray(deviceRunTime.starter_box_id, inactiveIds),
            isNull(deviceRunTime.end_time)
          )
        );
 ...
 */
export async function getMotorsForStarterControl(starterId: number) {
  return await db.query.motors.findMany({
    where: and(
      eq(motors.starter_id, starterId),
      ne(motors.status, "ARCHIVED"),
