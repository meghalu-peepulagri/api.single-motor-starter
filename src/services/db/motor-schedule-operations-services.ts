...
type ScheduleOperation = "CREATE" | "STOP" | "RESTART" | "DELETE";
...
export async function updateOperationAck(
  scheduleId: number,
  operation: ScheduleOperation,
  ackStatus: number,
) {
  const latest = await db.query.motorScheduleOperations.findFirst({
    where: and(
      ...
      eq(motorScheduleOperations.operation, operation),
    ),
...
export async function findLatestOperation(scheduleId: number, operation?: ScheduleOperation) {
...
export async function findOperationsByScheduleId(scheduleId: number) {
