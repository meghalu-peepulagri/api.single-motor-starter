...
export const benchedStarterParameters = pgTable("benched_starter_parameters", {
  ...
  index("benched_starter_params_starter_id_idx").on(table.starter_id),
  index("benched_starter_params_motor_id_idx").on(table.motor_id),
  index("benched_starter_params_starter_motor_idx").on(table.motor_id, table.starter_id),
]);

export type BenchedStarterParameters = typeof benchedStarterParameters.$inferSelect;
export type NewBenchedStarterParameters = typeof benchedStarterParameters.$inferInsert;
export type BenchedStarterParametersTable = typeof benchedStarterParameters;

export const benchedStarterParametersRelations = relations(benchedStarterParameters, ({ one }) => ({
  starter: one(starterBoxes, {
    fields: [benchedStarterParameters.starter_id],
    references: [starterBoxes.id],
  }),

  motor: one(motors, {
    fields: [benchedStarterParameters.motor_id],
    references: [motors.id],
    relationName: "motorParameters",
  }),

  gateway: one(gateways, {
    fields: [benchedStarterParameters.gateway_id],
    references: [gateways.id],
  }),
  user: one(users, {
    fields: [benchedStarterParameters.user_id],
    references: [users.id],
  })
}));