...
export const starterSettings = pgTable("starter_settings", {
  ...
  lrr: real("lrr").default(10),                 // Locked Rotor Recovery Time for Motor
  cir: real("cir").default(10),                 // Current Imbalance Recovery Time for Motor

  // Primary fault enable
  ...
  index("starter_settings_idx").on(table.starter_id),
]);


export type StarterSettings = typeof starterSettings.$inferSelect;
export type NewStarterSettings = typeof starterSettings.$inferInsert;
export type StarterSettingsTable = typeof starterSettings;

export const starterSettingsRelations = relations(starterSettings, ({ one }) => ({

  starter: one(starterBoxes, {
    fields: [starterSettings.starter_id],
